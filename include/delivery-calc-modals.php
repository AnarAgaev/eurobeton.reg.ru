<div class="modal" id="modalCoordsError">
    <div class="coords-error flex-column justify-content-center align-items-center" id="msgCoordsError">
        <h3 class="send-msg-true__title">Некорректный адерса доставки</h3>
        <p class="send-msg-true__txt">
            Для определения Адреса доставки воспользуйтесь картой.
            <b>Кликните на то место в котором Вы хотите получить продукцию</b>, после чего координаты
            этого места будут автоматически скопированы в поле Адрес дастовки.
        </p>
        <button class="btn" id="btnCloseCoordsError">Закрыть</button>
    </div>
</div>
<div class="modal" id="modalToLongRoute">
    <div class="coords-error flex-column justify-content-center align-items-center" id="msgToLongRoute">
        <h3 class="send-msg-true__title">Слишком длинный путь доставки</h3>
        <p class="send-msg-true__txt">
            К сожалению, в связи с технологическими особенностями продукции, доставка в указанное место невозможна.<br>
            Укажите другое место доставки.
        </p>
        <button class="btn" id="btnToLongRoute">Закрыть</button>
    </div>
</div>
